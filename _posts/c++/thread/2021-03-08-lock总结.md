...
| 自旋锁(spinlock)        | - `.lock()`<br>- `.unlock()`<br>- `.try_lock()`<br>- `std::lock_guard<>`<br>- `std::unique_lock<adopt_lock/defer_lock>` |
...
class Spinlock {
 public:
  ...
  void lock() {
    // memory_order_acquire: 获取锁时确保其后的内存操作不会被重排序到锁之前
  ...
  void unlock() {
    // memory_order_release: 释放锁时确保其前的内存操作不会被重排序到锁之后  
...
- 如果不提供第二参数，表示<u>构造时同时也上锁</u>，<u>析构时自动解锁（RAII特性）</u>
- 也可以手动控制锁的获取和释放

  ```c++
  std::unique_lock<std::mutex> some_lock(some_mutex);
  // do something
  some_lock.unlock();  // 可选：手动解锁
  // 离开作用域时，如果未手动解锁，析构函数会自动解锁
  ```
...
- **优先使用 `std::lock_guard`**：如果只需要简单的RAII锁管理
- **需要灵活控制时使用 `std::unique_lock`**：需要手动lock/unlock、条件变量、或延迟锁定
- **读多写少场景使用 `shared_mutex`**：可以显著提升并发读取性能
- **短临界区使用自旋锁**：当确定锁持有时间很短时（微秒级）
- **递归调用场景使用 `recursive_mutex`**：但应优先考虑重构代码避免递归锁定

### 3.2 性能考虑

| 锁类型            | 开销    | 适用场景               |
| ----------------- | ------- | ---------------------- |
| `std::mutex`      | 低      | 通用场景，推荐默认选择 |
| `Spinlock`        | CPU密集 | 极短临界区（<几微秒）  |
| `shared_mutex`    | 中等    | 读多写少场景           |
| `recursive_mutex` | 高      | 递归调用场景           |
